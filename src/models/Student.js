...
const studentSchema = new mongoose.Schema(
  {
    ...
    parentPhone: { type: String, trim: true, default: '', validate: { validator: (value) => !value || /^\+998\d{9}$/.test(value), message: 'Ota-ona telefoni +998XXXXXXXXX formatida bo‘lishi kerak' } },
    ...
    jshr: { type: String, required: true, trim: true, unique: true, match: /^\d{14}$/ },
    passportSeries: { type: String, required: true, trim: true, uppercase: true, match: /^[A-Z]{2}$/ },
    passportNumber: { type: String, required: true, trim: true, match: /^\d{7}$/ },
  },
  { timestamps: true },
)

studentSchema.index({ passportSeries: 1, passportNumber: 1 }, { unique: true })
